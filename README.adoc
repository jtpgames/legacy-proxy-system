...
Docker uses an embedded DNS server for container name resolution, typically running at `127.0.0.11:53` inside each container. Under high load conditions, this DNS resolver can become overwhelmed:

* **Limited Connection Handling**: The embedded DNS server has finite connection and query processing capacity
* **No Built-in Caching**: Each DNS query from the application layer results in a fresh lookup to the Docker DNS server
* **UDP-Based Resolution**: Docker DNS uses UDP, which under high concurrency can lead to packet drops or delayed responses
* **Shared Resource**: All containers on the same Docker network share the same DNS infrastructure
