...
* **High broker load:** Under sustained high throughput / heavy broker load, HiveMQ (CE and Edge) may become *sticky* and route almost everything to a single subscriber for an extended period (effectively disabling load balancing). See <<hivemq-cpu-bottleneck>>. 
...
* We have an unstable M/M/1 system, even though we try to have an M/M/c system. Over an experiment duration of 1h48m: Arrival rate λ = 302.5107 msg/s, Service rate μ = 22.3637 msg/s, Utilization ρ  = 13.5268
...
* HiveMQ CE issue (still open): https://github.com/hivemq/hivemq-community-edition/issues/558
